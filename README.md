...
| `presentation/` | The class talk (PDF + PPTX). |
...
> Weights are included (`model/weights/`, `model/coreml/`), so the apps run out of
> the box. Override with `$NANOPITCH_CHECKPOINT` / `$NANOPITCH_SRC_DIR` if needed.
