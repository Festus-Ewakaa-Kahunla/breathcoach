...
def load_labeled_clip(audio_path: Path, label_path: Optional[Path] = None) -> LabeledClip:
    ...
    if label_path.exists():
        ...
        # Hard negatives: DROPPED from training as of v15.
        # Ablation (v13 vs v14-skim vs v14-clean on GTSinger Tenor-1) showed that
        # up-weighting hard-negative regions — even with careful per-event human
        # review — REDUCED event-F1 (0.669 -> 0.621) by suppressing recall
        # (0.965 -> 0.907, 9 -> 24 missed breaths). The model became gun-shy.
        # The reviewed hard_negatives remain in the label JSON (dataset artifact +
        # documented negative result), but training no longer weights them: those
        # frames are ordinary label=0 background, exactly as in the v13 recipe.
        # To re-enable for future experiments, set HARD_NEG_WEIGHT > 1.0.
        HARD_NEG_WEIGHT = 1.0
        if HARD_NEG_WEIGHT != 1.0:
            REVIEWED_SOURCES = {
                "human_accepted", "human_added", "human_recategorized", "human_nudged",
                "festus_skim_accepted",
            }
            for ev in label_data.get("hard_negatives", []):
                if ev.get("source") not in REVIEWED_SOURCES:
                    continue
                start_frame = int(round(ev["start_sec"] * SAMPLE_RATE / HOP_SAMPLES))
                end_frame = int(round(ev["end_sec"] * SAMPLE_RATE / HOP_SAMPLES))
                start_frame = max(0, min(n_frames, start_frame))
                end_frame = max(0, min(n_frames, end_frame))
                confidence[start_frame:end_frame] = HARD_NEG_WEIGHT
...
N_EXTRA_FEATURES = 3  # ZCR, variance-of-mel, spectral-flatness


def compute_breath_features(waveform: np.ndarray, mel: np.ndarray,
                            hop_samples: int = HOP_SAMPLES,
                            window_samples: int = WINDOW_SAMPLES) -> np.ndarray:
    """Per-frame breath-noise features aligned with `compute_log_mel`.

    Returns (n_frames, 3): zero-crossing rate (broadband-noise cue),
    variance across mel bands per frame (peakiness vs flatness), and
    spectral flatness (geometric/arithmetic-mean of mel power). All three
    are standardised per-clip so each channel has mean 0 / std 1.
    """
    n_frames = mel.shape[0]
    feats = np.zeros((n_frames, 3), dtype=np.float32)
    for t in range(n_frames):
        s = waveform[t * hop_samples: t * hop_samples + window_samples]
        if s.size > 1:
            feats[t, 0] = float(np.mean(np.abs(np.diff(np.sign(s)))) * 0.5)
    feats[:, 1] = np.var(mel, axis=1).astype(np.float32)
    mel_pow = np.exp(mel.astype(np.float64))
    arith = mel_pow.mean(axis=1) + 1e-10
    geo = np.exp(np.log(mel_pow + 1e-10).mean(axis=1))
    feats[:, 2] = (geo / arith).astype(np.float32)
