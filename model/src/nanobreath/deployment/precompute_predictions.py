...
from nanobreath.data.dataset import (
    ...
    N_EXTRA_FEATURES, SAMPLE_RATE, HOP_SAMPLES,
)
from nanobreath.baseline.ruinskiy_lavner import RuinskiyDetector

# Allow our own checkpoints (which include pathlib.PosixPath in 'args') to load
...
def load_breath_head(ckpt_path: Path) -> tuple[BreathHead, int]:
    ...
    use_extra = args_dict.get("use_extra_features", False)
    ...
    head.eval()
    # Store calibration / finetuned-backbone / extra-features flag on the head.
    head._calibration = ckpt.get("calibration")
    head._backbone_state = ckpt.get("backbone_state_dict")
...
def main():
    ...
    if getattr(head, "_backbone_state", None) is not None:
        nanopitch.load_state_dict(head._backbone_state)
        print(f"  Swapped in finetuned backbone shipped with {args.breath_head.name}")
    detector = RuinskiyDetector()
