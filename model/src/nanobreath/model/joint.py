...
class JointModel(nn.Module):
    ...
    def forward(self, mel: torch.Tensor,
        ...
        Args:
            ...
            extra_features: optional ``(B, T, E)`` of breath-noise features
                (e.g. ZCR/VMS/spectral-flatness) concatenated to the backbone
                output before the breath head. The vad and pitch heads are
                unaffected.
        ...
        head_in = cat if extra_features is None else torch.cat([cat, extra_features], dim=-1)
        breath = self.breath_head(head_in)
