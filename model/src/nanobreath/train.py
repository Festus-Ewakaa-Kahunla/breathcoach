...
from nanobreath.data.dataset import (
    LabeledClip, collect_labeled_clips, compute_log_mel, compute_breath_features,
    SAMPLE_RATE, HOP_SAMPLES, N_MELS, N_EXTRA_FEATURES,
)
...
def parse_args():
    ...
    p.add_argument("--finetune-backbone", action="store_true",
                   help="unfreeze NanoPitch and train end-to-end (target: breath)")
    p.add_argument("--backbone-lr", type=float, default=1e-4,
                   help="LR for NanoPitch when --finetune-backbone is set")
    p.add_argument("--use-extra-features", action="store_true",
                   help="feed ZCR + variance-of-mel + spectral-flatness into the head")
...
class BreathDataset(Dataset):
    ...
    def __init__(self, clips: List[LabeledClip], seq_len: int, training: bool = True,
        ...
        self._features = (
            [compute_breath_features(c.waveform, m) for c, m in zip(clips, self._mels)]
    ...
    def __getitem__(self, idx):
        ...
        if self.use_extra:
            f_crop = self._features[clip_idx][start:start + self.seq_len].astype(np.float32)
            return (torch.from_numpy(mel_crop),
                    torch.from_numpy(f_crop),
...
def train(args):
    ...
    train_ds = BreathDataset(
        ...
        use_extra=args.use_extra_features,
    )
    ...
    val_features = (
        [compute_breath_features(c.waveform, m).astype(np.float32)
         for c, m in zip(val_clips, val_mels)]
        if args.use_extra_features else None
    )
    ...
    if args.finetune_backbone:
        for p in nanopitch.parameters():
            p.requires_grad = True
        print(f"Backbone UNFROZEN for end-to-end finetune "
              f"(backbone-lr={args.backbone_lr}, head-lr={args.lr})")
    head_in_features = 384 + (N_EXTRA_FEATURES if args.use_extra_features else 0)
    breath_head = BreathHead(in_features=head_in_features, hidden=args.hidden).to(device)
    print(f"BreathHead params: {breath_head.num_parameters():,}")
    joint = JointModel(nanopitch, breath_head).to(device)

    if args.finetune_backbone:
        optimizer = torch.optim.AdamW([
            {"params": breath_head.parameters(), "lr": args.lr},
            {"params": nanopitch.parameters(), "lr": args.backbone_lr},
        ])
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            _vad, _pitch, breath = joint(mel, extra_features=feats)
        ...
        with torch.no_grad():
            for i, (mel, lbls) in enumerate(zip(val_mels, val_labels_arrs)):
                mel_t = torch.from_numpy(mel).unsqueeze(0).to(device)  # (1, T, 40)
                feats_t = None
                if args.use_extra_features and val_features is not None:
                    feats_t = torch.from_numpy(val_features[i]).unsqueeze(0).to(device)
                _vad, _pitch, breath = joint(mel_t, extra_features=feats_t)
        ...
        if all_probs:
            ...
            if fm.pr_auc > best_pr_auc:
                ...
                if args.finetune_backbone:
                    ckpt_save["backbone_state_dict"] = nanopitch.state_dict()
                torch.save(ckpt_save, best_path)
