...
from nanobreath.data.dataset import compute_log_mel, compute_breath_features, SAMPLE_RATE, HOP_SAMPLES
...
_models = {"nanopitch": None, "head": None, "hidden": 8, "detector": None,
           ...
           "start_time": time.time(), "model_version": "unknown"}


def parse_multipart(body: bytes, content_type: str) -> dict[str, bytes]:
    """Minimal multipart/form-data parser. Returns {field_name: bytes_content}.

    Sufficient for the iOS app, which sends a single `audio` field. Not a full
    RFC 7578 implementation — no charset detection, no nested fields, no
    ...
    for part in body.split(delimiter):
        ...
        try:
            ...
            continue
        # name="..." lives in Content-Disposition
        name: str | None = None
...
def process_audio(waveform: np.ndarray, sample_rate: int,
    ...
    extra_t = None
    if getattr(_models["head"], "_use_extra", False):
        feats = compute_breath_features(waveform, mel)
    ...
    with torch.no_grad():
        vad, pitch, breath = joint(mel_t, extra_features=extra_t)
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            self._send_json({
                "ok": True,
                "model_version": _models["model_version"],
                "uptime_sec": round(time.time() - _models["start_time"], 2),
            })
            return
    ...
    def _handle_detect(self):
        """POST /detect — multipart `audio` field → DetectResponse schema.

        Wraps process_audio so the existing /process route stays unchanged for
        the browser UI. The iOS app's BreathAPI hits this endpoint.
        """
        try:
            n = int(self.headers.get("Content-Length", "0"))
            ctype = self.headers.get("Content-Type", "")
            if n <= 0:
                self._send_json({"error": "Empty body", "code": "INVALID_AUDIO"}, 400); return
            if n > 50 * 1024 * 1024:
                self._send_json({"error": "Too large (>50 MB)", "code": "TOO_LONG"}, 413); return

            body = self.rfile.read(n)
            if "multipart/form-data" not in ctype:
                self._send_json({"error": "Expected multipart/form-data",
                                 "code": "INVALID_AUDIO"}, 400); return

            fields = parse_multipart(body, ctype)
            audio_bytes = fields.get("audio")
            if not audio_bytes:
                self._send_json({"error": "Missing 'audio' field",
                                 "code": "INVALID_AUDIO"}, 400); return

            waveform, sr = decode_audio(audio_bytes)
            if sr != SAMPLE_RATE:
                waveform = resample_linear(waveform, sr, SAMPLE_RATE)

            result = process_audio(waveform, SAMPLE_RATE, spec_png_path=None)
            # Translate predicted_events ({start_sec, end_sec, score}) to the
            # DetectResponse contract ({start_sec, end_sec, confidence}).
            events = [{"start_sec": e["start_sec"],
                       "end_sec":   e["end_sec"],
                       "confidence": e.get("score", e.get("confidence", 0.0))}
                      for e in result.get("predicted_events", [])]
            self._send_json({
            ...
            import traceback
            traceback.print_exc()
            self._send_json({"error": str(exc), "code": "MODEL_UNAVAILABLE"}, 500)
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Save the WAV + spectrogram into ./recordings/ so the browser can
            # fetch them via <audio src=...> for playback. Filename based on timestamp.
            stamp = int(time.time() * 1000)
            web_dir = Path(__file__).parent / "recordings"
...
def main():
    ...
    # OSS layout: weights live under ../model/weights (gitignored — bring your own
    # backbone; see web/README.md). Fall back to config's env-var resolution.
    weights_dir = Path(__file__).resolve().parents[1] / "model" / "weights"
    default_bh = weights_dir / "breathcoach_v13.pth"
    default_np = weights_dir / "nanopitch" / "best.pth"

    p = argparse.ArgumentParser()
    p.add_argument("--port", type=int, default=8421)
    p.add_argument("--nanopitch", type=Path,
                   default=(default_np if default_np.exists() else cfg.NANOPITCH_CHECKPOINT),
                   help="NanoPitch backbone checkpoint. Defaults to "
                        "../model/weights/nanopitch/best.pth (or $NANOPITCH_CHECKPOINT).")
    p.add_argument("--breath-head", type=Path,
                   default=(default_bh if default_bh.exists() else None),
                   help="Trained BreathHead checkpoint. Defaults to "
                        "../model/weights/breathcoach_v13.pth.")
    ...
    if args.nanopitch is None:
        sys.exit("No NanoPitch backbone found. Place model.py + best.pth in "
                 "model/weights/nanopitch/, or pass --nanopitch / set "
                 "$NANOPITCH_CHECKPOINT. (The backbone is gitignored — see web/README.md.)")
    if args.breath_head is None:
        sys.exit("No BreathHead checkpoint found. Pass --breath-head, or place it at "
                 "model/weights/breathcoach_v13.pth.")
    ...
    if getattr(_models["head"], "_backbone_state", None) is not None:
        _models["nanopitch"].load_state_dict(_models["head"]._backbone_state)
        print(f"  Swapped in finetuned backbone from {args.breath_head.name}")
    ...
    # Version string surfaced via /health — derived from the BreathHead's run folder.
    _models["model_version"] = args.breath_head.parent.name if args.breath_head else "unknown"
    _models["start_time"] = time.time()
    print(f"Models loaded ({sum(p.numel() for p in _models['head'].parameters())} BreathHead params)")
    print(f"Model version: {_models['model_version']}")
    ...
    print(f"  GET  /health    {{ok, model_version, uptime_sec}}")
    print(f"  POST /process   upload audio, get predictions JSON")
    print(f"  POST /detect    multipart `audio` → DetectResponse (iOS app)")
